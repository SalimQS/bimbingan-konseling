...
<?php
  ...
  if(isset($_POST['changeFoto'])) {
    ...
    if(($_FILES['foto']['size'] >= $maxSize)) {
      ...
      $connect->begin_transaction();
      $query = "UPDATE data SET foto='$file' WHERE id_siswa = '". $_GET['id'] ."'";
      $result = $connect->query($query);
      if($result) {
                    ...
                    if($result2->num_rows > 0) {
                        ...
                        while($kelas = $result2->fetch_assoc()) {
                          if($user['kelas_id'] == $kelas['id']) {
                              echo('<option value="'.$kelas['id'].'" selected>'.$kelas['kelas'].'</option>');
                          }
                          else echo('<option value="'.$kelas['id'].'">'.$kelas['kelas'].'</option>');
                        }
                    }
